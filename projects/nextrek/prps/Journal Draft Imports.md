...
- 當對應的 form 表單，內容有做任何變動後，若使用者要離開當下頁面，會出現瀏覽器原生的確認提示，向使用者確認是否真的要離開「要離開網站嗎？系統可能不會儲存你做的變更」
- 若是正常的送出動作，就不會跳出確認提示
- 需要考量以下的情況：
	- 表單的內容欄位可能是前端動態產生，但如 add nested field，後續增加的欄位，一樣要有偵測到變動後，在離開頁面時會觸發提示的行為
	- 欄位也有可能是用 turbo frame 載入/更新的內容
- 另外可提供
	- 有 api 或方法可以強制清除偵測到的變動狀態

可以目前的資金帳戶暫存區 (draft_stages#show) 畫面為套用目標，讓它可以正確套用這個 unload blocking 的功能，確保包括
- 有勾選資料或是 input 的資料有異動 會觸發 unload blocking 提示。
- 若是正常的送出，不會觸發：
	- 單筆/多筆資料的送出儲存
	- 單筆/多筆資料的刪除

並且這個元件是要可以在未來套用到類似場景的表單使用
請詳細評估擬定計畫再開始實作
若有任何問題或建議請提出與我討論
