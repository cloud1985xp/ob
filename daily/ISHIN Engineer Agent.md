...
- 從特定的清單，取得多個不同專案的開發項目 issue，完成每個專案的開發 issue
- 執行程式部署
...
每日(早上)執行
有一組程式(agent)，先將各項工作來源，收集成一份本日工作事項
並且擬定工作計畫，讓我先 review，我可以追加、刪除或調整項目
並且討論區分哪些項目要由 agent 代理執行
產生最後的工作計畫

工作項目來源可以包括
- 來自多個不同 github repository，被打上特定 label 的 issues
- 查詢我的 google calendar，一些特定名稱的事件
...
最後主 agent 將所有 subagent 的工作結果紀錄，匯整成
